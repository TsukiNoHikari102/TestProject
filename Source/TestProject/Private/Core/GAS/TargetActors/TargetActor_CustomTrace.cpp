...
ATargetActor_CustomTrace::ATargetActor_CustomTrace()
{
...
void ATargetActor_CustomTrace::ResetSpread()
{
	bUseAimingSpreadMod = false;
	BaseSpread = 0.0f;
	AimingSpreadMod = 0.0f;
...
float ATargetActor_CustomTrace::GetCurrentSpread() const
{
...
void ATargetActor_CustomTrace::SetStartLocation(const FGameplayAbilityTargetingLocationInfo& InStartLocation)
{
	StartLocation = InStartLocation;
}

void ATargetActor_CustomTrace::SetShouldProduceTargetDataOnServer(bool bInShouldProduceTargetDataOnServer)
{
	ShouldProduceTargetDataOnServer = bInShouldProduceTargetDataOnServer;
}

void ATargetActor_CustomTrace::SetDestroyOnConfirmation(bool bInDestroyOnConfirmation)
{
	bDestroyOnConfirmation = bInDestroyOnConfirmation;
}

void ATargetActor_CustomTrace::StartTargeting(UGameplayAbility* Ability)
{
...
void ATargetActor_CustomTrace::ConfirmTargetingAndContinue()
{
...
void ATargetActor_CustomTrace::CancelTargeting()
{
...
void ATargetActor_CustomTrace::BeginPlay()
{
	Super::BeginPlay();
...
void ATargetActor_CustomTrace::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	DestroyReticleActors();
...
void ATargetActor_CustomTrace::Tick(float DeltaSeconds)
{
...
void ATargetActor_CustomTrace::LineTraceWithFilter(TArray<FHitResult>& OutHitResults, const UWorld* World,
...
void ATargetActor_CustomTrace::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params,
...
bool ATargetActor_CustomTrace::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection,
...
void ATargetActor_CustomTrace::StopTargeting()
{
	SetActorTickEnabled(false);
...
FGameplayAbilityTargetDataHandle ATargetActor_CustomTrace::MakeTargetData(const TArray<FHitResult>& HitResults) const
{
...
TArray<FHitResult> ATargetActor_CustomTrace::PerformTrace(AActor* InSourceActor)
{
	...
	if (bUsePersistentHitResults)
	{
		...
		for (int32 i = PersistentHitResults.Num() - 1; i >= 0; i--)
		{
			...
			if (HitResult.bBlockingHit || !IsValid(HitResult.GetActor()) || FVector::DistSquared(
...
AGameplayAbilityWorldReticle* ATargetActor_CustomTrace::SpawnReticleActor(FVector Location, FRotator Rotation)
{
	if (ReticleClass)
	{
...
void ATargetActor_CustomTrace::DestroyReticleActors()
{
